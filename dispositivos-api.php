...
function obtener_token_acceso() {
    ...
    $client_id = $_ENV['AIRALO_CLIENT_ID'] ?? null;
    $client_secret = $_ENV['AIRALO_CLIENT_SECRET'] ?? null;
    ...
    // Almacenar el token en una transiente con un margen de seguridad (1 hora menos)
    $expiration_time = max($expires_in - 3600, 0);
    set_transient('airalo_access_token', $token, $expiration_time);
